 ...
 *  ...........................................
 *  Version: 1.5b "Sandra" / 2020-06-06
...
typedef struct m_pack {
    ...
    uint8_t rx_byte_count;                                  /* Bytes received in RX buffer */
    uint8_t rx_head;
    uint8_t rx_tail;
    uint8_t tx_head;
    uint8_t tx_tail;
    OverflowState device_state;
    uint8_t command[MST_PACKET_SIZE * 2];
    uint8_t data_byte;
...
#if CMD_READFLASH
inline static void Reply_READFLSH(MemPack *p_mem_pack) __attribute__((always_inline));
#endif /* CMD_READFLASH */
inline static void Reply_INITSOFT(MemPack *p_mem_pack) __attribute__((always_inline));
void InitPack(MemPack *p_mem_pack);

// USI TWI driver prototypes
void UsiTwiTransmitByte(MemPack *p_mem_pack);
uint8_t UsiTwiReceiveByte(void);
inline static void UsiTwiDriverInit(MemPack *p_mem_pack) __attribute__((always_inline));
inline static void TwiStartHandler(MemPack *p_mem_pack) __attribute__((always_inline));
...
//#pragma message "Prescaler dividing clock by 8, setting the CPU prescaler division factor to 1 ..."
    ...
    UsiTwiDriverInit(p_mem_pack);                                 /* Initialize the TWI driver */
        ...
        if (((p_mem_pack->flags >> FL_INIT_1) & true) && ((p_mem_pack->flags >> FL_INIT_2) & true)) {
#endif /* TWO_STEP_INIT */
            // =============================================
            // = Exit the bootloader & run the application =
            // =============================================
            ...
            */            
            if (slow_ops_enabled == true) {
                ...
                // ==================================================
                // = Delete the application from memory (Slow-Op 1) =
                // ==================================================
                ...
                // ===========================================================================
                // = Write the received page to memory and prepare for a new one (Slow-Op 2) =
                // ===========================================================================
...
inline void Reply_GETTMNLV(MemPack *p_mem_pack) {
    ...
    for (uint8_t i = 0; i < GETTMNLV_RPLYLN; i++) {
        p_mem_pack->data_byte = reply[i];
        UsiTwiTransmitByte(p_mem_pack);
    }
    return;
}
...
inline void Reply_STPGADDR(MemPack *p_mem_pack) {
    ...
    p_mem_pack->page_addr = ((p_mem_pack->command[1] << 8) + p_mem_pack->command[2]);   /* Sets the flash memory page base address */
    p_mem_pack->page_addr &= ~(SPM_PAGESIZE - 1);               /* Keep only pages' base addresses */
    reply[0] = AKPGADDR;
    reply[1] = (uint8_t)(p_mem_pack->command[1] + p_mem_pack->command[2]);              /* Returns the sum of MSB and LSB of the page address */
...
inline void Reply_WRITPAGE(MemPack *p_mem_pack) {
    ...
    if ((p_mem_pack->page_addr + p_mem_pack->page_ix) == RESET_PAGE) {
#if AUTO_PAGE_ADDR
        p_mem_pack->app_reset_lsb = p_mem_pack->command[1];
        p_mem_pack->app_reset_msb =  p_mem_pack->command[2];
#endif /* AUTO_PAGE_ADDR */
        ...
        reply[1] += (uint8_t)(( p_mem_pack->command[2]) +  p_mem_pack->command[1]);   /* Reply checksum accumulator */
        p_mem_pack->page_ix += 2;
        for (uint8_t i = 3; i < (MST_PACKET_SIZE + 1); i += 2) {
            boot_page_fill((p_mem_pack->page_addr + p_mem_pack->page_ix), (( p_mem_pack->command[i + 1] << 8) |  p_mem_pack->command[i]));
            reply[1] += (uint8_t)(( p_mem_pack->command[i + 1]) +  p_mem_pack->command[i]);
            p_mem_pack->page_ix += 2;
        }                
    } else {
...
#if CHECK_PAGE_IX
    if ((reply[1] !=  p_mem_pack->command[MST_PACKET_SIZE + 1]) || (p_mem_pack->page_ix > SPM_PAGESIZE)) {
#else
    if (reply[1] !=  p_mem_pack->command[MST_PACKET_SIZE + 1]) {
#endif /* CHECK_PAGE_IX */
...
// ******************
#if CMD_READFLASH
inline void Reply_READFLSH(MemPack *p_mem_pack) {
    const uint8_t reply_len = (p_mem_pack->command[3] + 2);             /* Reply length: ack + memory positions requested + checksum */
    ...
    const __flash uint8_t *mem_position;
    mem_position = (void *)((p_mem_pack->command[1] << 8) + p_mem_pack->command[2]);
    for (uint8_t i = 1; i < p_mem_pack->command[3] + 1; i++) {
        reply[i] = (*(mem_position++) & 0xFF);              /* Actual memory position data */
        reply[reply_len - 1] += (uint8_t)(reply[i]);        /* Checksum accumulator */
    }
    reply[reply_len - 1] += (uint8_t)(p_mem_pack->command[1]);          /* Add Received address MSB to checksum */
    reply[reply_len - 1] += (uint8_t)(p_mem_pack->command[2]);          /* Add Received address MSB to checksum */
...
void InitPack(MemPack *p_mem_pack) {
    //MemPack mem_pack;
    p_mem_pack = (MemPack*)malloc(sizeof(MemPack));
...
#endif /* AUTO_PAGE_ADDR */
    p_mem_pack->rx_byte_count = 0;
    p_mem_pack->rx_head = 0;
    p_mem_pack->rx_tail = 0;
    p_mem_pack->tx_head = 0,
    p_mem_pack->tx_tail = 0;
}
...
*/
void UsiTwiTransmitByte(MemPack *p_mem_pack) {
    p_mem_pack->tx_head = ((p_mem_pack->tx_head + 1) & TWI_TX_BUFFER_MASK); /* Update the TX buffer index */
    while (p_mem_pack->tx_head == p_mem_pack->tx_tail) {};          /* Wait until there is free space in the TX buffer */
    p_mem_pack->tx_buffer[p_mem_pack->tx_head] = p_mem_pack->data_byte;         /* Write the data byte into the TX buffer */
}
...
/*  _______________________________
   ...
   |_______________________________|
*/
void UsiTwiDriverInit(MemPack *p_mem_pack) {
    // Initialize USI for TWI Slave mode.
    p_mem_pack->tx_tail = p_mem_pack->tx_head = 0;                  /* Flush TWI TX buffers */
    p_mem_pack->rx_tail = p_mem_pack->rx_head = p_mem_pack->rx_byte_count = 0;  /* Flush TWI RX buffers */
...
/*  _______________________________________________________
   ...
   |_______________________________________________________|
*/
inline void TwiStartHandler(MemPack *p_mem_pack) {
    ...
    p_mem_pack->device_state = STATE_CHECK_RECEIVED_ADDRESS;
...
inline bool UsiOverflowHandler(MemPack *p_mem_pack) {    
    switch (p_mem_pack->device_state) {
        ...
        case STATE_CHECK_RECEIVED_ADDRESS: {
            if ((USIDR == 0) || ((USIDR >> 1) == TWI_ADDR)) {
                if (USIDR & 0x01) {     /* If data register low-order bit = 1, start the send data mode */
                    // >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
                    //                                                                   >>
                    uint8_t command_size = p_mem_pack->rx_byte_count;               //                 >>
                    //static uint8_t command[MST_PACKET_SIZE * 2];        //                   >>
                    for (uint8_t i = 0; i < command_size; i++) {        //  Call a function    >>
                        while (p_mem_pack->rx_byte_count-- == 0) {};                //  in main to process   >>
                        p_mem_pack->rx_tail = ((p_mem_pack->rx_tail + 1) & TWI_RX_BUFFER_MASK); //  the received data    >>
                        p_mem_pack->command[i] = p_mem_pack->rx_buffer[p_mem_pack->rx_tail];                //  (command) ...      >>
                    }                                                   //                   >>
                    ReceiveEvent(p_mem_pack);    //                 >>
                    //                                                                   >>
                    // >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
                    // Next state -> STATE_SEND_DATA_BYTE
                    p_mem_pack->device_state = STATE_SEND_DATA_BYTE;
                } else {                /* If data register low-order bit = 0, start the receive data mode */
                    // Next state -> STATE_RECEIVE_DATA_BYTE
                    p_mem_pack->device_state = STATE_RECEIVE_DATA_BYTE;
                }
                SET_USI_TO_SEND_ACK();
            } else {
        ...
        case STATE_SEND_DATA_BYTE: {
            if (p_mem_pack->tx_head != p_mem_pack->tx_tail) {
                // If the TX buffer has data, copy the next byte to USI data register for sending                
                p_mem_pack->tx_tail = ((p_mem_pack->tx_tail + 1) & TWI_TX_BUFFER_MASK);
                USIDR = p_mem_pack->tx_buffer[p_mem_pack->tx_tail];
            } else {
            ...
            p_mem_pack->device_state = STATE_RECEIVE_ACK_AFTER_SENDING_DATA; 
        ...
        case STATE_RECEIVE_ACK_AFTER_SENDING_DATA: {
            ...
            p_mem_pack->device_state = STATE_CHECK_RECEIVED_ACK;
        ...
        case STATE_RECEIVE_DATA_BYTE: {
            ...
            p_mem_pack->device_state = STATE_PUT_BYTE_IN_RX_BUFFER_AND_SEND_ACK;
        ...
        case STATE_PUT_BYTE_IN_RX_BUFFER_AND_SEND_ACK: {
            ...
            p_mem_pack->rx_byte_count++;
            p_mem_pack->rx_head = ((p_mem_pack->rx_head + 1) & TWI_RX_BUFFER_MASK);
            p_mem_pack->rx_buffer[p_mem_pack->rx_head] = USIDR;
            // Next state -> STATE_RECEIVE_DATA_BYTE
            p_mem_pack->device_state = STATE_RECEIVE_DATA_BYTE;
            SET_USI_TO_SEND_ACK();
            return false;
