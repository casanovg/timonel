...
int main() {
    ...
    */
    ...
    Usi_onReceivePtr = ReceiveEvent;        /* Pointer to TWI receive event function */
    Usi_onRequestPtr = RequestEvent;        /* Pointer to TWI request event function */
    //p_flags = &flags;
    __SPM_REG = (_BV(CTPB) | \
                 _BV(__SPM_ENABLE));        /* Clear temporary page buffer */
    asm volatile("spm");
    byte enable_slow_ops = false;           /* Run slow operations only after completing TWI handshake */
    ...
    for (;;) {
        ...
        */
        if ((USISR & (1 << USI_OVERFLOW_FLAG)) && (USICR & (1 << USI_OVERFLOW_INT))) {
            ...
            enable_slow_ops = UsiOverflowHandler(TWI_ADDR);   
        }
