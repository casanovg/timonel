...
The bootloader has several optional features that allow finding the right balance between characteristics, flash memory usage, and performance. They're enabled from the configuration files **"tml-t85-*.ini"** placed inside the "configs" folder (e.g. "tml-t85-std.ini", the default one). Each ".ini" file defines a build environment and adds or removes sections of the source code that will be part of the binary file when compiling it, through the "build_flags" settings. As a general rule of thumb, more enabled features = bigger bootloader size = less space for user applications. The available options are briefly described below:
